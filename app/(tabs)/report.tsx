...
export default function ReportIssue() {
  ...
  const handleAddMedia = async () => {
    ...
    if (!perm.granted) {
      Alert.alert(
        t('report.permissionNeeded'),
        t('report.permissionDesc'),
      );
      return;
    }
  ...
  const handleSubmit = async () => {
    if (!subject.trim()) {
      Alert.alert(
        t('report.subjectRequired'),
        t('report.subjectRequiredDesc'),
      );
      return;
    }
    ...
    if (!description.trim()) {
      Alert.alert(
        ...
        t('report.descriptionRequiredDesc'),
      );
      return;
    }
    ...
    try {
      ...
      media.forEach((m, i) => {
        const ext =
          m.uri.split('.').pop()?.toLowerCase() ?? 'jpg';
      ...
      if (!res.ok) {
        throw new Error(
          data?.error || t('report.failedTitle'),
        );
      }

      // ── Report successfully sent ──
      // Clear/reset all form data only after successful submission
      setCategory('Fraud / Scam');
      setSubject('');
      setDescription('');
      setMedia([]);
      setFollowUp(true);
