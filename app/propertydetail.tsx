...
function formatPrice(
  ...
  if (freq === "For Sale")
    return { amount: `${f} XAF`, period: t("listing.forSale") };
  if (freq === "Yearly")
    return { amount: `${f} XAF`, period: t("propertyDetail.yearlyPeriod") };
...
function buildLegacyNearbyPlaces(
  ...
  const restaurant =
    listing.nearbyRestaurantName ?? listing.nearby_restaurant_name ?? null;
  ...
  const places: LegacyNearbyPlace[] = [
    ...
    {
      label: t("propertyDetail.nearbyRestaurant"),
      example: restaurant,
      icon: "coffee",
    },
    {
      label: t("propertyDetail.nearbyMarket"),
      example: market,
      icon: "shopping-bag",
    },
    {
      label: t("propertyDetail.nearbyClinic"),
      example: clinic,
      icon: "activity",
    },
  ];
          ...
          {sorted.map((img, i) => (
            <TouchableOpacity key={i} onPress={() => setActiveIndex(i)}>
              <Image
                ...
                style={[
                  styles.fsThumb,
                  i === activeIndex && styles.fsThumbActive,
                ]}
                resizeMode="cover"
              />
            </TouchableOpacity>
      ...
      <View style={styles.sectionBlock}>
        <View style={styles.sectionTitleRow}>
          ...
          <Text style={styles.sectionTitle}>
            {t("propertyDetail.videoWalkthrough")}
          </Text>
        </View>
        <View style={styles.videoWrap}>
          ...
          <View style={styles.videoPill}>
            ...
            <Text style={styles.videoPillTxt}>
              {t("propertyDetail.videoComingSoon")}
            </Text>
          </View>
        </View>
      </View>
...
export default function PropertyDetailScreen() {
  ...
  const handleContact = async () => {
    ...
    try {
      ...
      if (!convRes.ok) {
        ...
        throw new Error(
          (errBody as any).error ||
            "Could not start conversation (HTTP " + convRes.status + ").",
        );
      }
      ...
      // This file is supplied with the fix as app/MessagesScreen.tsx.
      router.push({
    ...
    return (
      <SafeAreaView style={styles.centered}>
        ...
        <Text style={styles.errorTxt}>
          {error ?? t("propertyDetail.notFound")}
        </Text>
  ...
  const location = [listing.neighborhood, listing.city, listing.region]
    .filter(Boolean)
    .join(", ");
  ...
  const stats = [
    {
      icon: "home",
      label: t("propertyDetail.parlour"),
      val: listing.parlors ?? listing.parlour ?? "—",
    },
    {
      icon: "book",
      label: t("propertyDetail.beds"),
      val: listing.bedrooms ?? "—",
    },
    {
      icon: "droplet",
      label: t("propertyDetail.baths"),
      val: listing.bathrooms ?? "—",
    },
    {
      icon: "wind",
      label: t("propertyDetail.toilets"),
      val: listing.toilets ?? "—",
    },
    {
      icon: "coffee",
      label: t("propertyDetail.kitchen"),
      val: listing.kitchens ?? listing.kitchen ?? "—",
    },
  ] as const;
        ...
        <View style={styles.heroOverlay}>
          ...
          <View style={styles.heroRightBtns}>
            <TouchableOpacity
              style={styles.heroBtn}
              onPress={async () => {
                const ok = await toggleFavourite(id as string);
                if (!ok) {
                  Alert.alert(
                    t("favourites.loginTitle"),
                    t("favourites.loginDesc"),
                    [
                      { text: t("common.cancel"), style: "cancel" },
                      {
                        text: t("auth.login"),
                        onPress: () =>
                          router.push("/house_seekers_login_signup" as any),
                      },
                    ],
                  );
                }
        ...
        <View style={styles.body}>
                ...
                <View>
                  ...
                  <Text style={styles.rentalVal}>
                    {listing.minRentalPeriod ??
                      t("propertyDetail.minRentalDefault")}
                  </Text>
                </View>
              </View>
              <TouchableOpacity style={styles.applyBtn}>
                <Text style={styles.applyBtnTxt}>
                  {t("propertyDetail.applyNow")}
                </Text>
              </TouchableOpacity>
            </View>
          )}
          ...
          <View style={styles.sectionBlock}>
            <View style={styles.sectionTitleRow}>
              <View style={styles.sectionDot} />
              <Text style={styles.sectionTitle}>
                {t("propertyDetail.propertyDescription")}
              </Text>
            </View>
          ...
          {Array.isArray(listing.facilities) && listing.facilities.length > 0 && (
            <View style={styles.sectionBlock}>
              <View style={styles.sectionTitleRow}>
                <View style={styles.sectionDot} />
                <Text style={styles.sectionTitle}>
                  {t("propertyDetail.facilitiesAmenities")}
                </Text>
              </View>
                  ...
                  >
                    ...
                    {img.isPrimary && (
                      <View style={styles.primaryBadge}>
                        <Text style={styles.primaryBadgeTxt}>
                          {t("propertyDetail.main")}
                        </Text>
                      </View>
                    )}
                  </TouchableOpacity>
