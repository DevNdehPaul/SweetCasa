...
digital booking authorization.`
    const transactionSummary = `<div class="box"><b>Property:</b> ${escapeHtml(listing.title)}<br><b>Seeker/Tenant:</b> ${escapeHtml(seeker?.name || seeker?.email)}<br><b>Landlord:</b> ${escapeHtml(listing.owner?.name || listing.owner?.email)}<br><b>Protected amount:</b> ${amounts.required.toLocaleString()} XAF<br>${amounts.kind === 'RENT' ? `<b>Initial rent:</b> ${amounts.rentAmount.toLocaleString()} XAF<br><b>Caution fee:</b> ${amounts.cautionFeeAmount.toLocaleString()} XAF<br><b>Move-in / key-handover date:</b> ${moveInDate.toISOString().slice(0,10)}<br><b>Day 8 standard release date:</b> ${protectionEndsAt.toISOString().slice(0,10)}<br><b>Standard platform commission:</b> 5% of rent only<br>` : `<b>Purchase commitment:</b> 25% of listed sale price<br>`}</div>`
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>${title}</title><style>body{font-family:Arial,sans-serif;max-width:860px;margin:40px auto;padding:0 24px;color:#24183a;line-height:1.55}h1{color:#6d28d9}.box{background:#f6f2ff;padding:16px;border-radius:12px;margin-bottom:24px}.terms{font-family:Arial,sans-serif;white-space:pre-wrap;word-wrap:break-word;font-size:14px;line-height:1.6}.sig{margin-top:32px;border-top:1px solid #bbb;padding-top:16px}</style></head><body>
      <h1>${title}</h1>${transactionSummary}${agreementBody}
      <div class="sig"><b>Tenant electronic signature:</b> ${escapeHtml(signatureName)}<br><b>Signed:</b> ${signedAt.toISOString()}<br><b>Transaction ID:</b> E${hold.id}<br><b>Payment authorization:</b> Required funds moved from available balance to SweetCasa held balance.</div></body></html>`
    let document = null
    try {
