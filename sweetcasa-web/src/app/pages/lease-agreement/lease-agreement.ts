...
digital booking authorization.`;
 ...
 ngOnInit(){const id=Number(this.route.snapshot.paramMap.get('id'));if(!id){this.error.set('Property not found.');this.loading.set(false);return};this.signatureName=this.auth.profile()?.name||'';this.api.get<any>(`/listings/${id}`).subscribe({next:r=>{this.listing.set(r.listing||r);this.loadWallet()},error:e=>{this.error.set(e.error?.error||'Could not load this property.');this.loading.set(false)}})}
 loadWallet(){this.api.get<any>('/wallet/me').subscribe({next:r=>{this.wallet.set(r.wallet);this.loading.set(false)},error:e=>{this.error.set(e.error?.error||'Could not load your escrow wallet.');this.loading.set(false)}})}
 isSale(){return this.listing()?.paymentFrequency==='For Sale'} rent(){return this.isSale()?0:Number(this.listing()?.price||0)} caution(){return this.isSale()?0:Number(this.listing()?.cautionFee||0)} required(){const p=Number(this.listing()?.price||0);return this.isSale()?Math.ceil(p*.25):p+this.caution()} available(){return Number(this.wallet()?.availableBalance||0)} remaining(){return this.available()-this.required()} shortfall(){return Math.max(0,this.required()-this.available())} ownerName(){const l=this.listing();return l?.agent?.name||l?.owner?.name||l?.ownerName||'Verified House Owner'} seekerName(){return this.auth.profile()?.name||this.auth.profile()?.fullName||this.signatureName||'House Seeker'} location(){const l=this.listing();return [l?.neighborhood,l?.city,l?.region,l?.country].filter(Boolean).join(', ')} minDate(){return new Date().toISOString().slice(0,10)}
 sign(){if(this.shortfall()>0){this.error.set(`Your available escrow balance is short by ${this.shortfall().toLocaleString()} XAF.`);return}if(!this.isSale()&&!this.moveInDate){this.error.set('Enter the expected move-in / key-handover date.');return}if(this.signatureName.trim().length<2){this.error.set('Enter your full name as your electronic signature.');return}if(!this.accepted){this.error.set('Please confirm that you have reviewed and accept the agreement.');return}this.busy.set(true);this.error.set('');this.api.post<any>('/wallet/purchase',{listingId:this.listing().id,signatureName:this.signatureName.trim(),moveInDate:this.isSale()?this.minDate():this.moveInDate}).subscribe({next:r=>{this.busy.set(false);this.success.set(true);this.agreementUrl.set(r.agreement?.url||'');this.api.clearCache('/wallet')},error:e=>{this.busy.set(false);this.error.set(e.error?.error||'Could not complete the protected transaction.')}})}
 download(){const u=this.agreementUrl();if(u)window.open(u,'_blank','noopener,noreferrer')}
}
